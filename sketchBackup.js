...
function draw() {
  ...
  if (frameCount % 30 == 0 && numberOfFaces != 0) {
    console.log(faces[1]);
...
function DrawFace(face) {
  ...
  if (face.state === brfv4.BRFState.FACE_TRACKING_START || face.state === brfv4.BRFState.FACE_TRACKING) {
    ...
    for (var i = 0; i < face.vertices.length; i += 2) {
      fDS[i / 2] = [
        face.vertices[i] - center[0],
        face.vertices[i + 1] - center[1]
      ];
    }
    console.log(fDS[0][0]);

    let head = [
      0, 0, fDS[16][0] - fDS[0][0],
      (fDS[8][1] - fDS[29][1]) * 2
    ];
    ...
    bezier(fDS[48][0], fDS[48][1] * 0.95, fDS[67][0], fDS[67][1], fDS[65][0], fDS[65][1], fDS[54][0], fDS[54][1] * 0.95,);
    bezier(fDS[48][0], fDS[48][1] * 0.95, fDS[61][0], fDS[61][1], fDS[63][0], fDS[63][1], fDS[54][0], fDS[54][1] * 0.95,);
    pop();
    ...
    for (var i = 0; i < fDS.length; i++) {
      fill(255, 0, 0);
      rect(fDS[i][0], fDS[i][1], 10, 10);
      text(fDS[i][0], fDS[i][0], fDS[i][1],)
      text(fDS[i][1], fDS[i][0], fDS[i][1] + 20,)
    }
